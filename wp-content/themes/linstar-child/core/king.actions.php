...
function king_save_page_layout_template( $post_id, $post ) {
	...
	if( $post->post_type == 'our-team' ){
		...
		$email = !empty($_POST['king_staff_email']) ? $_POST['king_staff_email'] : '';
...
?>
	...
	<table>
		...
		<tr>
			<td>
				<label><?php _e('Email',KING_DOMAIN); ?>: </label>
			</td>
			<td>	
				<input type="text" name="king_staff_email" value="<?php echo esc_attr( $staff['position'] );	?>" />
			</td>
		</tr>
		<tr>
			<td>
